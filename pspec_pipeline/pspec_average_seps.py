...
def perform_sum(dsum=None, weights=None, kpl=None, values=None,
    ...
    weights = thing by which the sum needs to be divided to get an answer (this is _not_ done in this function)
...
for filename in args.files:
    ...
    for pspec_key, error_key in zip(flat_power_spectra, flat_errors):
        summed_pspec[pspec_key], summed_weights[error_key], summed_vars[error_key] = perform_sum(kpl=f['kpl'],
                                                          values=f[pspec_key],
                                                          errors=f[error_key],
    ...
    for pspec_key, error_key in zip(folded_power_spectra, folded_errors):
        summed_pspec[pspec_key], summed_weights[error_key], summed_vars[error_key] = perform_sum(kpl=f['kpl_fold'],
                                                          values=f[pspec_key],
                                                          errors=f[error_key],
                                                          dsum=summed_pspec[pspec_key],
                                                          vsum=summed_vars[error_key],
...
for pspec_key, error_key in zip(flat_power_spectra, flat_errors):
    out_dict[pspec_key] = np.array([summed_pspec[pspec_key][_k]
                               / summed_weights[error_key][_k] for _k in kpl])
...
for pspec_key, error_key in zip(folded_power_spectra, folded_errors):
    out_dict[pspec_key] = np.array([summed_pspec[pspec_key][_k]
                               / summed_weights[error_key][_k] for _k in kpl_folds])
    out_dict[error_key] = np.array([np.sqrt(summed_vars[error_key][_k]
                            / summed_weights[error_key][_k]) for _k in kpl_folds])
