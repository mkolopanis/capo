...
"""Compute Signal Loss factor via distribution method v6.
    ...
    -For each injection level, a 1D KDE is used to smooth out the points (P_in columns)
...
def smooth_dist(fold=True):
    ...
    for kk,k in enumerate(ks):
        ...
        for col in range(n_injects): # loop over injection levels
            ...
            kernel_C = scipy.stats.gaussian_kde(ys_kde[col]) # KDE for 1D data
            kernel_I = scipy.stats.gaussian_kde(ys_I_kde[col]) # KDE for 1D data
