#! /usr/bin/env python
"""Helpful functions for returning current EoR results and handling data."""
import numpy as n
import numpy as np
import os
...
def PAPER_32_all():
    """Return results from  PAPER 32  Jacobs et.al 2014.

    outputs results[z] = n.array([k, Delta^2, 2-sigma upper, 2-sigma lower])
    """
    PAPER_RESULTS_FILES = (glob.glob(os.path.dirname(__file__)
                           + '/data/psa32_apj/pspec_*.npz'))
...
def MWA_32T_all():
    ...
    TREATMENT:
    I will store the delta^2 +/- values.
    If he doesn't give me a delta^2 or lower limit, I will put
    Delta^2 = 0 and bottom = -top

    return format will be dict[z] = n.array([[k,Delta^2,top,bottom]])
    all in mK^2

    """
    MWA_RESULTS_FILE = (os.path.dirname(__file__)
                        + '/data/MWA_32T_Results_Summary.txt')
    ...
    for line in lines:
        if line.endswith('2sigUL'):  # if its just an upper limit
            ...
            # if its a "detection" which just means its not consistent with 0,
            #  not really EoR but probably foregrounds or systematics
            l = map(float, line.split()[:-1])
            z = l[-1]
            try:
...
def MWA_128_all():
    """
    MWSA_128 data from dillon 2015.

    return format will be dict[z] = n.array([[k,Delta^2,top,bottom]])
    all in mK^2
    """
    MWA_RESULTS_FILE = (glob.glob(os.path.dirname(__file__)
                        + '/data/mwa128/*.dat'))
    zs = []
    results = {}
...
def MWA_32T_at_z(redshift):
    """
    Split MWA32T data near redshift.

    Input a redshift
    Return the Delta^2(k) power spectrum at the redshift
    nearest the input redshift
...
def MWA_32T_at_k(k):
    ...
    Input a k
    Returns the Delta^2(k) power spectrum vs redshift at that k.
    (all pspec in mk^2)
...
def average_bootstraps(indata, Nt_eff, avg_func=n.median, Nboots=100, version=4):
    ...
    then get the mean and standard deviation across scrambles
...
def read_injects(inj_dirs=None):
    """
    Read boots in inject directories.
    ...
    returns:
        dictionary of outputs from read_bootstraps,
        keys are the inject_directory names.
    """
...
def random_avg_bootstraps(boot_dict=None, boot_axis=None, time_axis=None,
    ...
    """
    Performa random averaging of bootstraps.
